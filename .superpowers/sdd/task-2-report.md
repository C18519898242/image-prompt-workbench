...
- 为示例图补齐内联 `width: "100%"`、`height: "100%"` 与已有的 `objectFit: "contain"`，确保固定 4:3 容器内的实际呈现受完整约束。
- 为参考图缩略图沿用相同的宽高与 `contain` 约束，未增加任何交互或参数功能。
- 测试同时断言五个下拉框的完整且唯一 option value 集合，并保留既有默认值断言。

### 验证结果

先运行工作台测试，新增图片尺寸断言在修复前失败，确认 RED；修复后运行：
...
结果：工作台测试 5/5 通过；全量前端测试 27/27 通过；差异检查退出码 0，无空白错误。

### 自审

- 仅修改生成工作台组件、测试与本任务报告，未改 CSS、App、API 或后端。
- 未加入未确认参数、清空、字数、快捷键、拖拽、裁剪、进度、结果或历史；页面仍只有一个“开始生成”按钮。
