...
- 来源：提示词资料或远程提示词卡片
- 用途：浏览、对照和理解提示词的生成效果
- 默认状态：只读，不会自动参与新的生成请求
- 交互：可以查看同一提示词下的多张示例图，也可以由用户主动选入生成参考图

### 生成参考图

指用户在一次生图任务中主动上传或选择的图片，用于约束人物、场景、产品、构图或其他视觉特征。

- 来源：用户本地上传，或由用户从提示词示例图中主动选择
- 用途：作为本次生成请求的输入条件
- 默认状态：可编辑、可移除，是否参与生成由用户决定
- 交互：可以添加、替换和删除；生成请求发送时随提示词一起提交

两者的关系是：**提示词示例图用于展示效果，生成参考图用于参与生成**。除非用户主动选择，否则提示词示例图不会被当作生成参考图使用。
...
- 新增或编辑提交的最终图片清单累计像素不超过 8000 万，服务端统一格式转换后的累计暂存输出不超过 200 MB；这两项均属于服务端资源保护限制；
- 标题、提示词正文和图片顺序数据分别不超过 64 KiB（按 UTF-8 编码后的字节数计算）；
- 编辑时可以移除、追加和调整图片顺序；
...
`app.import_prompt_cards` 用于从远程 `README_zh.md` 解析提示词卡片，下载提示词示例图，并写入 SQLite 数据库。运行前请先完成上面的数据表初始化。
...
```
#### 🖼️ 生成图片
<img src="https://example.com/example.png" alt="提示词示例图">
````
